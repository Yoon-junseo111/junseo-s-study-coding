...
model.add(Dropout(0,3))   # 몇 프로의 노드를 이번 훈련에서 제거 Dropout
...
#  CPU  걸린시간 :  42.12 초   epochs = 1000

#  GPU  걸린시간 :  12.2 초
#-----------------------------------------------
# CPU 걸린시간 : 37.99 초  epochs = 200

# GPU 걸린시간 : 62.18 초


# R2 score를 제외한 나머지 rmse, mae 등은 낮아야 좋음 이유: loss값이 낮아야 되기 때문
# 훈련 횟수, 데이터가 많을수록 GPU가 더 빠름

# x = [0,2,3,5만,10]
# minmax = [0,0.1,0.0.01,0.05,1] 
# y = [1,2,3,4,5]
# 정규화는 y 건들지말기 (x 데이터만)
# 데이터의 위치와 속성은 그대로
...
#   ---------     <--- 공식
#    max-min             
#  (train의 최댓값 - train의 최솟값)
                          # 첫 train 데이터를 minmax로 잡는다
                         # 전체 데이터가 0 ~ 100만이고
                         # train 데이터가 0 ~ 1
                         # test 데이터가 0.1~ 1.1
                         # predict (예측값) : 0 ~ 110만
