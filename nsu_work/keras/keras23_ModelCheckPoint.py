...
#2. 모델
model=Sequential()
model.add(Dense(200, activation ='relu',input_shape=(8, )))     # activation = 'relu' -> 활성화 함수 설정
...
#   Earlystopping : 성능이 좋아질 때까지는 계속 학습하고, 더 이상 좋아지지 않으면 자동으로 종료하라.
es = EarlyStopping(
    monitor='val_loss',   # val_loss를 모니터링 한다
    mode='min',    # 좋아지는 기준 설정 (val_loss 낮을수록 좋음 )
    patience=1000,   #  10 번까지 학습 기다림 (훈련동안 개선이 지속되면 계속 훈련 진행)
...
model.fit(x_train, y_train, epochs = 1000000000, batch_size = 1000, 
                            validation_split=0.2, 
                            callbacks = [es,mcp]
                            )                    # Earlystopping 적용 , loss 값이 가장 낮은 곳에서 저장
