...
model.add(Dense(3)) #입력값이 400000개인 데이터를 받아, 뉴런이 3개인 완전 연결층을 모델에 추가한다.
model.add(Dense(2))
...
model.fit(x, y , epochs = 10000, batch_size=4)# 훈련횟수   # batch_size = 한 번에 모델이 학습하는 데이터의 개수, 훈련이 빨리지는 효과
